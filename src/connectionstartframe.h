...
public:
	 ...
	 */
	ConnectionStartFrame(uint8_t major, uint8_t minor, const Table &properties, const std::string &mechanisms
	                     , const std::string &locales)
		:
		ConnectionFrame((uint32_t)(properties.size() + mechanisms.length() + locales.length() + 10))
		, // 4 for each longstring (size-uint32), 2 major/minor
		_major(major)
		, _minor(minor)
	...
	virtual bool process(ConnectionImpl *connection) override {
		...
		connection->send(
			ConnectionStartOKFrame(properties, connection->login().mechanismRepr(), connection->login().stringRepr()
			                       , "en_US"));
