...
uint16_t ConnectionImpl::add(const std::shared_ptr<ChannelImpl> &channel) {
	...
	if (_maxChannels > 0 && _channels.size() >= _maxChannels) {
		return 0;
	}
	...
	while (true) {
		// is this id in use?
		if (_nextFreeChannel > 0 && _channels.find(_nextFreeChannel) == _channels.end()) {
			break;
		}
...
uint64_t ConnectionImpl::parse(const Buffer &buffer) {
	...
	if (!_closed || _state != state_connected) {
		return processed;
	}
...
bool ConnectionImpl::close() {
	...
	if (_closed || _state == state_closed) {
		return false;
	}
	...
	if (waiters > 0 || _state != state_connected) {
		return true;
	}
...
void ConnectionImpl::setReady() {
	...
	if (_closed && _state == state_connected && !waiting()) {
		sendClose();
	}
}
