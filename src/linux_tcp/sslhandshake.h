...
private:
	...
	TcpState *nextstate(const Monitor &monitor) {
		...
		if (allowed) {
			return new SslConnected(this, std::move(_ssl), std::move(_out));
		}
...
public:
	...
	SslHandshake(TcpExtState *state, const std::string &hostname, TcpOutBuffer &&buffer)
		...
		if (OpenSSL::SSL_set_fd(_ssl, _socket) == 0) {
			throw std::runtime_error("failed to associate filedescriptor with ssl socket");
		}
	 ...
	 *  @return std::size_t
	 */
	std::size_t queued() const override { return _out.size(); }
	...
	TcpState *process(const Monitor &monitor, int fd, int flags) override {
		...
		case SSL_ERROR_WANT_READ:
			return proceed(readable);
		case SSL_ERROR_WANT_WRITE:
			return proceed(readable | writable);
		default:
