...
void DeferredExtReceiver::initialize(const std::string &exchange, const std::string &routingkey) {
	...
	if (_messageCallback) {
		_message.construct(exchange, routingkey);
	}
}
...
void DeferredExtReceiver::complete() {
	...
	if (_message) {
		_messageCallback(*_message, _deliveryTag, _redelivered);
	}

	// do we have to inform anyone about completion?
	if (_deliveredCallback) {
		_deliveredCallback(_deliveryTag, _redelivered);
	}
