...
protected:
	...
	void append(const void *data, size_t size) override {
		...
		if (_size > 0 && _size + size > 4096) {
			flush();
		}

		// if data would not fit anyway, we send it immediately
		if (size > 4096) {
			return _handler->onData(_connection, (const char *)data, size);
		}
...
public:
	 ...
	 *  @param  frame
	 */
	PassthroughBuffer(Connection *connection, ConnectionHandler *handler, const Frame &frame)
		: _buffer()
		, _size(0)
