...
protected:
	 ...
	 *  @return Deferred        Next deferred result
	 */
	virtual const std::shared_ptr<Deferred> &
	reportSuccess(const std::string &name, uint32_t messagecount, uint32_t consumercount) const {
	 ...
	 */
	virtual const std::shared_ptr<Deferred> &
	reportSuccess(uint32_t messagecount, uint64_t deliveryTag, bool redelivered) {
		// make sure compilers dont complain about unused parameters
		(void)messagecount;
		(void)deliveryTag;
		(void)redelivered;
...
public:
	 ...
	 */
	explicit Deferred(bool failed = false)
		: _failed(failed) {}
...
public:
	...
	Deferred &onError(const ErrorCallback &callback) {
		...
		if (_failed) {
			callback("Frame could not be sent");
		}
