...
private:
	...
	public:
		 ...
		 */
		Watcher(struct ev_loop *loop, Watchable *object, int fd, int events)
			: _loop(loop) {
	...
	private:
		...
		virtual void onActive(int fd, int events) override {
			...
			if (_timeout != 0 && (events & EV_READ)) {
				_expire = ev_now(_loop) + _timeout * 1.5;
			}
...
public:
	 ...
	 */
	LibEvHandler(struct ev_loop *loop)
		: _loop(loop) {}
