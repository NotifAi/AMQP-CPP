...
private:
	...
	reportSuccess(const std::string &name, uint32_t messagecount, uint32_t consumercount) const override {
		// skip if no special callback was installed
		if (!_queueCallback) {
			return Deferred::reportSuccess();
		}
...
public:
	 ...
	 */
	DeferredQueue(bool failed = false)
		: Deferred(failed) {}
