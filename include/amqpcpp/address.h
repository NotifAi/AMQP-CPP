...
public:
	 ...
	 */
	Address(const char *data, size_t size)
		: _vhost("/") {
		...
		if (at != nullptr) {
			...
			// assign the login
			_login = Login(
				std::string(data, colon ? colon - data : loginsize), std::string(colon ? colon + 1 : "", colon ? at
				                                                                                                 - colon
				                                                                                                 - 1
				                                                                                               : 0)
			);
		...
		// was a vhost set?
		if (slash != nullptr && last - slash > 1) {
			_vhost.assign(slash + 1, last - slash - 1);
		}
	...
	bool operator==(const Address &that) const {
		...
		if (_secure != that._secure) {
			return false;
		}

		// logins must match
		if (_login != that._login) {
			return false;
		}

		// hostname must match, but are not case sensitive
		if (strcasecmp(_hostname.data(), that._hostname.data()) != 0) {
			return false;
		}
