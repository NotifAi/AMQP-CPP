 ...
 *  Class definition
 */
class TcpConnection : private ConnectionHandler, private Watchable, private TcpParent {
private:
	/**
	 ...
	 */
	void onProperties(Connection *connection, const Table &server, Table &client) override;
	 ...
	 */
	uint16_t onNegotiate(Connection *connection, uint16_t interval) override;
	 ...
	 */
	void onReady(Connection *connection) override {
	 ...
	 *  @param  state
	 */
	void onConnected(TcpState *state) override {
		// pass on to the handler
		_handler->onConnected(this);
	}

	void onSetupSecurity(TcpState *state, SSL *ssl) override {
	 ...
	 *  @return bool
	 */
	bool onSecured(TcpState *state, const SSL *ssl) override {
		// pass on to user-space
	 ...
	 *  @return size_t
	 */
	size_t onReceived(TcpState *state, const Buffer &buffer) override {
	 ...
	 *  @param  events
	 */
	void onIdle(TcpState *state, int socket, int events) override {
		// pass on to user-space
	 ...
	 *  @param  connected
	 */
	void onError(TcpState *state, const char *message, bool connected) override;

	/**
	 *  Method to be called when it is detected that the connection was lost
	 *  @param  state
	 */
	void onLost(TcpState *state) override;

	/**
	 *  The expected number of bytes
	 *  @return size_t
	 */
	size_t expected() override {
		// pass on to the connection
