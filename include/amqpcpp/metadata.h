...
public:
	...
	MetaData(ReceivedFrame &frame)
		...
		if (hasContentType()) {
			_contentType = ShortString(frame);
		}
		if (hasContentEncoding()) {
			_contentEncoding = ShortString(frame);
		}
		if (hasHeaders()) {
			_headers = Table(frame);
		}
		if (hasDeliveryMode()) {
			_deliveryMode = UOctet(frame);
		}
		if (hasPriority()) {
			_priority = UOctet(frame);
		}
		if (hasCorrelationID()) {
			_correlationID = ShortString(frame);
		}
		if (hasReplyTo()) {
			_replyTo = ShortString(frame);
		}
		if (hasExpiration()) {
			_expiration = ShortString(frame);
		}
		if (hasMessageID()) {
			_messageID = ShortString(frame);
		}
		if (hasTimestamp()) {
			_timestamp = Timestamp(frame);
		}
		if (hasTypeName()) {
			_typeName = ShortString(frame);
		}
		if (hasUserID()) {
			_userID = ShortString(frame);
		}
		if (hasAppID()) {
			_appID = ShortString(frame);
		}
		if (hasClusterID()) {
			_clusterID = ShortString(frame);
		}
	}
	...
	void setDeliveryMode(uint8_t value) {
		_deliveryMode = value;
		_bools1.set(4, true);
	}

	void setHeaders(const Table &value) {
		_headers = value;
		_bools1.set(5, true);
	}
	 ...
	 */
	void setExpiration(std::string &&value) {
		_expiration = std::move(value);
		_bools1.set(0, true);
	}

	void setReplyTo(std::string &&value) {
		_replyTo = std::move(value);
		_bools1.set(1, true);
	}

	void setCorrelationID(std::string &&value) {
		_correlationID = std::move(value);
		_bools1.set(2, true);
	}

	void setHeaders(Table &&value) {
		_headers = std::move(value);
		_bools1.set(5, true);
	}

	void setContentEncoding(std::string &&value) {
		_contentEncoding = std::move(value);
		_bools1.set(6, true);
	}
	...
	void setClusterID(std::string &&value) {
		_clusterID = std::move(value);
		_bools2.set(2, true);
	}

	void setAppID(std::string &&value) {
		_appID = std::move(value);
		_bools2.set(3, true);
	}

	void setUserID(std::string &&value) {
		_userID = std::move(value);
		_bools2.set(4, true);
	}

	void setTypeName(std::string &&value) {
		_typeName = std::move(value);
		_bools2.set(5, true);
	}
	...
	uint32_t size() const {
		...
		if (hasExpiration()) {
			result += (uint32_t)_expiration.size();
		}
		if (hasReplyTo()) {
			result += (uint32_t)_replyTo.size();
		}
		if (hasCorrelationID()) {
			result += (uint32_t)_correlationID.size();
		}
		if (hasPriority()) {
			result += (uint32_t)_priority.size();
		}
		if (hasDeliveryMode()) {
			result += (uint32_t)_deliveryMode.size();
		}
		if (hasHeaders()) {
			result += (uint32_t)_headers.size();
		}
		if (hasContentEncoding()) {
			result += (uint32_t)_contentEncoding.size();
		}
		if (hasContentType()) {
			result += (uint32_t)_contentType.size();
		}
		if (hasClusterID()) {
			result += (uint32_t)_clusterID.size();
		}
		if (hasAppID()) {
			result += (uint32_t)_appID.size();
		}
		if (hasUserID()) {
			result += (uint32_t)_userID.size();
		}
		if (hasTypeName()) {
			result += (uint32_t)_typeName.size();
		}
		if (hasTimestamp()) {
			result += (uint32_t)_timestamp.size();
		}
		if (hasMessageID()) {
			result += (uint32_t)_messageID.size();
		}
	...
	void fill(OutBuffer &buffer) const {
		...
		// only copy the properties that were sent
		if (hasContentType()) {
			_contentType.fill(buffer);
		}
		if (hasContentEncoding()) {
			_contentEncoding.fill(buffer);
		}
		if (hasHeaders()) {
			_headers.fill(buffer);
		}
		if (hasDeliveryMode()) {
			_deliveryMode.fill(buffer);
		}
		if (hasPriority()) {
			_priority.fill(buffer);
		}
		if (hasCorrelationID()) {
			_correlationID.fill(buffer);
		}
		if (hasReplyTo()) {
			_replyTo.fill(buffer);
		}
		if (hasExpiration()) {
			_expiration.fill(buffer);
		}
		if (hasMessageID()) {
			_messageID.fill(buffer);
		}
		if (hasTimestamp()) {
			_timestamp.fill(buffer);
		}
		if (hasTypeName()) {
			_typeName.fill(buffer);
		}
		if (hasUserID()) {
			_userID.fill(buffer);
		}
