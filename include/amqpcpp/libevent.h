...
private:
	...
	public:
		...
		void events(int events) {
			...
			// set the events
			event_assign(_event, event_get_base(_event), event_get_fd(_event), event_flags, event_get_callback(_event)
			             , event_get_callback_arg(_event));
...
public:
	 ...
	 */
	LibEventHandler(struct event_base *evbase)
		: _evbase(evbase) {}
